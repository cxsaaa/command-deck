...
export async function searchCommands(query: string, filter: CommandFilter): Promise<Command[]> {
...
export async function saveRelatedData(
  ...
  if (input.examples && input.examples.length > 0) {
    const filtered = input.examples.map((e) => e.trim()).filter((e) => e.length > 0);
      ...
      if (p.name.trim().length > 0 && p.description.trim().length > 0) {
        await db.execute(
          `INSERT INTO command_parameters (id, command_id, name, description, sort_order)
           VALUES ($1, $2, $3, $4, $5)`,
          [`${commandId}_param_${i}`, commandId, p.name.trim(), p.description.trim(), i],
        );
      }
    }
  }
