...
export async function updatePlatformSortOrder(platformIds: string[]): Promise<void> {
  const db = await getDb();
  for (let i = 0; i < platformIds.length; i++) {
    await db.execute("UPDATE platforms SET sort_index = $1, updated_at = $2 WHERE id = $3", [
      i,
...
export async function renamePlatform(id: string, name: string): Promise<void> {
  const db = await getDb();
  await db.execute("UPDATE platforms SET name = $1, updated_at = $2 WHERE id = $3", [
    name,
    new Date().toISOString(),
    id,
  ]);
...
export async function hidePlatform(id: string): Promise<void> {
  const db = await getDb();
  await db.execute("UPDATE platforms SET is_visible = 0, updated_at = $1 WHERE id = $2", [
    new Date().toISOString(),
    id,
  ]);
}
...
export async function showPlatform(id: string): Promise<void> {
  const db = await getDb();
  await db.execute("UPDATE platforms SET is_visible = 1, updated_at = $1 WHERE id = $2", [
    new Date().toISOString(),
    id,
  ]);
}
